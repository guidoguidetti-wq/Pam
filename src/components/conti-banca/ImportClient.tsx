...
export function ImportClient() {
  ...
  async function handleImport() {
    ...
    if (!banca.trim()) { toast.error('Inserisci il nome della banca'); return }
    ...
    try {
      ...
      fd.append('file', file)
      fd.append('banca', banca.trim())

      const res = await fetch('/api/conti-banca/import', { method: 'POST', body: fd })
      const data = await res.json()
      if (!res.ok) { toast.error(data.error ?? 'Errore importazione'); return }

      setResult(data)
      toast.success(`Importati ${data.imported} movimenti`)
    } catch {
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div className="rounded-lg border p-5 space-y-4">
        <h2 className="font-semibold text-base">Importa movimenti bancari</h2>

        <div className="space-y-1.5">
          <Label htmlFor="banca">Nome Banca *</Label>
          <Input
            id="banca"
            placeholder="Es. BPER Banca, Intesa Sanpaolo…"
            value={banca}
        ...
        <div className="rounded-md bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 p-3 text-xs text-blue-800 dark:text-blue-200 space-y-1">
          ...
          <p>File di estratto conto con colonne: Data operazione, Data valuta, Descrizione, Entrate, Uscite, Categoria, Stato.</p>
          <p>I movimenti già presenti vengono ignorati (nessun duplicato).</p>
        </div>

        <Button onClick={handleImport} disabled={loading || !file || !banca.trim()} className="w-full">
      ...
      {result && (
        <div className="rounded-lg border border-green-200 bg-green-50 dark:bg-green-950 dark:border-green-800 p-5 space-y-3">
          <div className="flex items-center gap-2 text-green-700 dark:text-green-400">
            ...
            <span className="font-semibold">Importazione completata</span>
          </div>
